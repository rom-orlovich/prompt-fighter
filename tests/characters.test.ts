...
describe('character specs', () => {
  ...
  it('resolves the shared clip library to its own asset', () => {
    expect(characterAssetUrl(ANIMATION_ASSET, '/')).toBe('/assets/characters/Anims.glb');
  });
  ...
  it('resolves every vendored asset to a local, same-origin .glb path under the app base', () => {
    for (const asset of [...BODIES, ...HAIRSTYLES, ANIMATION_ASSET]) {
      const url = characterAssetUrl(asset, '/');
      expect(url).toBe(`/assets/characters/${asset}.glb`);
      expect(url.startsWith('http')).toBe(false);
      expect(url.startsWith('//')).toBe(false);
    }
    expect(characterAssetUrl('Male', '/prompt-fighter/')).toBe(
      '/prompt-fighter/assets/characters/Male.glb'
    );
  });
  ...
  it('scales every fighter into the arena height band the camera is framed for', () => {
    ...
    // and the two bodies are not the same height on their own (1.82 vs 1.78).
  ...
  it('falls back to a generic character for an unknown model', () => {
    ...
    expect(BODIES as readonly string[]).toContain(unknown.body);
    expect(HAIRSTYLES as readonly string[]).toContain(unknown.hair);
